...
A normal user account authenticates with a password. TOTP (RFC 6238) is available as an optional second factor a user can enable on their own account — not required by default, consistent with the household-appliance-not-corporate-system stance already taken on approvals in Section 15.

Enrollment:

1. The Hub generates a random secret and presents it as a QR-encoded `otpauth://totp/...` URI, plus the same secret/URI as selectable text for manual entry into an authenticator app (needed on the Hub station's own screen, since a user can't scan their own display with their own phone).
2. The user enters one code generated by their app to confirm it before TOTP is actually enabled on the account.
3. A set of one-time backup codes is generated and shown once, for use if the authenticator app/device is lost.
